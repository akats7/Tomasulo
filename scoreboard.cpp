...
int main(int argc, char *argv[])
{
    ...
    while (done == 0)
    {
        ...
        {                                      //         IF LOAD FUNCTIONALITY
            ...
            if (LD_busy > ready)
            { // If the LD is busy after the destination reg is busy, the issue is the clock cycle of the LD is freed
                ready = LD_busy;
            }
            ...
            { // checks to make sure no Write after read hazard exists
        ...
        else if (instruction == "ADD")
        {
            ...
            { // checks for structural hazard
                ready = intreg_busy[dest];
            }
            if (ADD_busy > ready)
            { // checks is add is busy
                ready = ADD_busy;
            }
            ready++;
            cout << setw(2) << ready << " ";
